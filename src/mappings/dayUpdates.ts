...
import { Pair, Bundle, Token, CCSwapFactory, CCSwapDayData, PairDayData, TokenDayData } from '../types/schema'
import { ONE_BI, ZERO_BD, ZERO_BI, FACTORY_ADDRESS } from './helpers'

export function updateUniswapDayData(event: EthereumEvent): CCSwapDayData {
  let uniswap = CCSwapFactory.load(FACTORY_ADDRESS)
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400
  let dayStartTimestamp = dayID * 86400
  let uniswapDayData = CCSwapDayData.load(dayID.toString())
  if (uniswapDayData === null) {
    uniswapDayData = new CCSwapDayData(dayID.toString())
  ...
  return uniswapDayData as CCSwapDayData
}
